/**
 * The authoritative simulation of the global world, as the host PC runs it: the host
 * half of game/Game.ts with every player remote (restored from the removed node-server,
 * docs/pc-host-research.md §5.2). World edits, zombies, drops, crates, the clock
 ...
 *
 * No DOM, no rendering, no network: the PC's GameRoom feeds it messages and ticks it,
 * and the tests drive it directly. It speaks protocol v1, so today's web and Flutter
 * clients join it unchanged.
 */
import { World, type PropMeta } from '@game/world/chunkStore'
import { TerrainGenerator, WORLD_CHUNKS_Y } from '@game/world/terrainGen'
import { ChunkStreamer } from '@game/world/chunkStreamer'
import { raycastVoxels } from '@game/world/raycast'
import { AIR, BLOCK, isProp, isSolid } from '@game/world/palette'
import { makeRng } from '@game/world/noise'
import { PLAYER } from '@game/physics/playerController'
import { rayBox } from '@game/physics/aabb'
import { breakTime, dropForBlock, getItem } from '@game/items/registry'
import { craft, getRecipe } from '@game/items/recipes'
import type { ItemStack } from '@game/items/inventory'
import { DropManager } from '@game/entities/drops'
import { CrateManager } from '@game/entities/crates'
import { kindsForNight, ZombieManager, ZOMBIE, ZOMBIE_STATS, zombiesForNight, type Zombie } from '@game/entities/zombies'
import { Avatar, AVATAR } from '@game/game/Avatar'
import { DayNight } from '@game/game/DayNight'
import { rulesToWire, type GameRules } from '@game/game/rules'
import { nightsSurvived } from '@game/game/score'
import { collectSave, zombiesToRestore } from '@game/game/saveState'
import { Visitors } from '@game/game/visitors'
import type { SaveData } from '@game/net/api'
import { PROTOCOL_VERSION, type BlockEdit, type ClientMessage, type PrivateState, type Snapshot, type Welcome } from '@game/net/protocol'
...
export class HostSim {
  ...
  // ================================================================ players
  /** A player connected. One we have seen before (this run or in the save) gets their gear back. */
  ...
  welcome(a: Avatar): Welcome {
    return {
      ...
      edits: this.worldEdits(), spawn: { x: a.x, y: a.y, z: a.z },
  ...
  apply(a: Avatar, msg: ClientMessage): void {
    switch (msg.t) {
      ...
      // chat and hello are the room's business
      case 'chat':
      case 'hello':
        return
    }
