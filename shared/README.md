...
> **CI no longer checks this.** `client.yml` was dropped, so nothing in
> `.github/workflows/` runs `flutter test`. Drift between the two clients now
> fails only when someone runs `flutter test` in `client/` by hand — do that
> before merging anything that touches `shared/` or `client/`.

| Path | Produced by | Consumed by | Checks |
|---|---|---|---|
| `fixtures/worldgen/` | `npm run fixtures:worldgen` in `server/` — golden chunks, heights, trees, islands, updrafts, caves and ore veins for five seeds | `client/test/world/` | byte-identical chunks and templates |
