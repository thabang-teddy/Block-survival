...
export class DayNight {
  ...
  get night(): number {
    return this.time < DAY_SECONDS ? 0 : Math.floor((this.time - DAY_SECONDS) / CYCLE_SECONDS) + 1
  }

  phaseAt(t: number): Phase {
    return t % CYCLE_SECONDS < DAY_SECONDS ? 'day' : 'night'
  }

  /** seconds until the next sunset / dawn */
  get secondsToTransition(): number {
    const inCycle = this.time % CYCLE_SECONDS
    return inCycle < DAY_SECONDS ? DAY_SECONDS - inCycle : CYCLE_SECONDS - inCycle
  }

  /** 0..1 progress through the current night (0 during the day) */
  get nightProgress(): number {
    const inCycle = this.time % CYCLE_SECONDS
    return inCycle < DAY_SECONDS ? 0 : (inCycle - DAY_SECONDS) / NIGHT_SECONDS
  }
