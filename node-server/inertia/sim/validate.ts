/**
 * Every message a client sends is checked here before the host sim sees it. On a
 * peer-to-peer host a bad packet only hurt that one match; on the server every room
 * shares one process, so a malformed or hostile message is dropped rather than trusted.
 */
import type { ClientMessage } from '../net/protocol.ts'
import type { AnimName } from '../game/Avatar.ts'
import { HOTBAR_SIZE, INVENTORY_SIZE } from '../items/inventory.ts'
import { RECIPES } from '../items/recipes.ts'
