...
/** a WebRTC peer id; `server` is how this server marks its own rooms, so no peer may claim it */
...
/**
 * Rooms hosted peer-to-peer by a native client: room codes → the host's WebRTC peer id,
 * refreshed while the match runs. The web client plays on rooms this server runs
 * instead (PlayController in this folder); those rows are never touched from here.
 */
export default class RoomController {
  async store({ request, response, auth }: HttpContext) {
    ...
    // opening a room is when stale mail is swept
    await RoomSignal.pruneStale()

    const room = await Room.updateOrCreate(
      { code: data.code },
      {
        hostPeerId: data.host_peer_id,
        hostName: data.host_name,
        worldKind: 'own',
        userId: (auth.user as User | undefined)?.id ?? null,
        players: 1,
  ...
  /** resolve a code to its host: the host, a player with an accepted invite (issue #5), or anyone for the global world */
  ...
  /** the peer-to-peer host refreshes the TTL and player count while the match is running */
  async update({ params, request, response }: HttpContext) {
    ...
    if (!room) return response.notFound({ message: 'Not your room.' })
